...
export const handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    try {
      ...
      console.log('Blob 없음, 기본 데이터 반환:', err.message);
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
  ...
  if (event.httpMethod === 'POST') {
    try {
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ success: true, message: '저장 완료! 즉시 반영됩니다.' }),
      };
    } catch (err) {
      ...
      return {
        statusCode: 500,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: `저장 실패: ${err.message}` }),
      };
    }
  }
